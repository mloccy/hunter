...
function(hunter_pack_directory dir_to_pack dest_dir result_sha1 format)
  ...
  if (${format} EQUAL "${HUNTER_ARCHIVE_BZIP2_OPTION}" )
    if(HUNTER_STATUS_DEBUG AND NOT HUNTER_SUPPRESS_LIST_OF_FILES)
      list(APPEND cmd "cvjf")
    else()
      list(APPEND cmd "cjf")
    endif()
  elseif(${format} EQUAL "${HUNTER_ARCHIVE_GZIP_OPTION}")
    if(HUNTER_STATUS_DEBUG AND NOT HUNTER_SUPPRESS_LIST_OF_FILES)
      list(APPEND cmd "cvzf")
    else()
      list(APPEND cmd "czf")
    endif()
  elseif(${format} EQUAL "${HUNTER_ARCHIVE_LZMA_OPTION}")
    if(HUNTER_STATUS_DEBUG AND NOT HUNTER_SUPPRESS_LIST_OF_FILES)
      list(APPEND cmd "cvJf")
    else()
      list(APPEND cmd "cJf")
  ...
  set(dest_archive "${dest_dir}/${archive_sha1}.tar.${format}")
